...
const {handleClick_DownloadLatest} = require("./file_tab")
...
async function createWindow() {
    globals.mainWindow = new BrowserWindow({
        ...
        webPreferences: {
            ...
            persistUserDataDirName: 'PokeRogue-Kanto'
        }
    });
...
app.whenReady().then(() => {
    if (process.platform === 'darwin') {
        ...
        globals.gameDir = path.join(app.getPath('documents'), 'PokeRogue-Kanto', 'game');
    } else {
    ...
    createWindow();
      ...
      // Check if game files are found
  if (!globals.gameFilesDownloaded) {globals.mainWindow.setBackgroundColor("#000000"); handleClick_DownloadLatest();}
