    ...
    return new Promise((resolve, reject) => {
        utils.fetchLatestGameVersionInfo()
            .then(releaseData => {
                let zipAsset;
                //if (modded) {}
                zipAsset = releaseData.assets.find((asset) => asset.name === 'kanto_version.zip');
                ...
                if (zipAsset) {
                    ...
                    if(!downloadOngoing) {
                        ...
                        utils.downloadFile(zipUrl, zipPath, onBytesReceived)
                            .then(_ => {
                                ...
                                progressBar.detail = `Extracting... (Keep waiting even if it says "not responding")`;
                    ...
                    console.error('kanto_version.zip asset not found in the latest release');
                    reject(new Error('kanto_version.zip asset not found in the latest release.'));
                }
            })
            .catch(reason => {
                reject(reason);
...
module.exports.handleClick_DownloadLatest = handleClick_DownloadLatest;